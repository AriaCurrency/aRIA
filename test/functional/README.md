...
- The oldest supported Python version is specified in [doc/dependencies.md](/doc/dependencies.md).
  ...
  The Travis linter also checks this, but [possibly not in all cases](https://github.com/ariacurrency/aria/pull/14884#discussion_r239585126).
...
- Use a module-level docstring to describe what the test is testing, and how it
  is testing it.
- When subclassing the aRIATestFramework, place overrides for the
...
- Set the `self.setup_clean_chain` variable in `set_test_params()` to control whether
  ...
  has 25 mature blocks (25x50=1250 RIA) in its wallet.
...
with the aRIAd(s) being tested in a callback-based event loop; the other
implements the test logic.

- `P2PConnection` is the class used to connect to a aRIAd.  `P2PInterface`
contains the higher level logic for processing P2P payloads and connecting to
the aRIA Core node application logic. For custom behaviour, subclass the
...
The [`TestShell`](test-shell.md) class exposes the aRIATestFramework
...
Taken from the [python-aRIArpc repository](https://github.com/jgarzik/python-aRIArpc).
...
Test objects for interacting with a aRIAd node over the p2p interface.

#### [script.py](test_framework/script.py)
Utilities for manipulating transaction scripts (originally from python-aRIAlib)
