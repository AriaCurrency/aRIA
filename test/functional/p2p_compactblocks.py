...
# TestP2PConn: A peer we use to send messages to aRIAd, and store responses.
...
class CompactBlocksTest(aRIATestFramework):
    ...
    # This test actually causes aRIAd to (reasonably!) disconnect us, so do this last.
    ...
    # Test that aRIAd requests compact blocks when we announce new blocks
    ...
    def test_incorrect_blocktxn_response(self, test_node):
        ...
        # Note that it's possible for aRIAd to be smart enough to know we're
    ...
    def test_getblocktxn_handler(self, test_node):
        version = test_node.cmpct_version
        node = self.nodes[0]
        # aRIAd will not send blocktxn responses for blocks whose height is
