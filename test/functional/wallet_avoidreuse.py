...
class AvoidReuseTest(aRIATestFramework):
    ...
    def test_sending_from_reused_address_without_avoid_reuse(self):
        ...
        # listunspent should show 1 single, unused 10 RIA output
        assert_unspent(self.nodes[1], total_count=1, total_sum=10, reused_supported=True, reused_count=0)
        # getbalances should show no used, 10 RIA trusted
        ...
        # listunspent should show 1 single, unused 5 RIA output
        assert_unspent(self.nodes[1], total_count=1, total_sum=5, reused_supported=True, reused_count=0)
        # getbalances should show no used, 5 RIA trusted
        ...
        # listunspent should show 2 total outputs (5, 10 RIA), one unused (5), one reused (10)
        assert_unspent(self.nodes[1], total_count=2, total_sum=15, reused_count=1, reused_sum=10)
        # getbalances should show 10 used, 5 RIA trusted
        assert_balances(self.nodes[1], mine={"used": 10, "trusted": 5})

        self.nodes[1].sendtoaddress(address=retaddr, amount=10, avoid_reuse=False)

        # listunspent should show 1 total outputs (5 RIA), unused
        assert_unspent(self.nodes[1], total_count=1, total_sum=5, reused_count=0)
        # getbalances should show no used, 5 RIA trusted
        assert_balances(self.nodes[1], mine={"used": 0, "trusted": 5})

        # node 1 should now have about 5 RIA left (for both cases)
    ...
    def test_sending_from_reused_address_fails(self, second_addr_type):
        ...
        [0] sends 10 RIA to A.
        [1] spends 5 RIA from A. (leaving roughly 5 RIA useable)
        [0] sends 10 RIA to A again.
        [1] tries to spend 10 RIA (fails; dirty).
        [1] tries to spend 4 RIA (succeeds; change address sufficient)
        '''
