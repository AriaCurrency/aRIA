...
Verify that a aRIAd node can load multiple wallet files
"""
...
class MultiWalletTest(aRIATestFramework):
    ...
    def run_test(self):
        ...
        if self.options.descriptors:
            exp_stderr = r"Error: SQLiteDatabase: Unable to obtain an exclusive lock on the database, is it being used by another aRIAd?"
        else:
        ...
        if self.options.descriptors:
            assert_raises_rpc_error(-4, "Wallet file verification failed. SQLiteDatabase: Unable to obtain an exclusive lock on the database, is it being used by another aRIAd?", self.nodes[0].loadwallet, wallet_names[0])
        else:
