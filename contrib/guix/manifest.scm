...
(define* (make-aRIA-cross-toolchain target
                                  #:key
...
desirable for building aRIA Core release binaries."
  ...
  (let ((target (getenv "HOST")))
    (cond ((string-suffix? "-mingw32" target)
          ...
          ((string-contains target "riscv64-linux-")
           (list (make-aRIA-cross-toolchain "riscv64-linux-gnu"
                                               #:base-gcc-for-libc gcc-7)))
          ((string-contains target "-linux-")
           (list (make-aRIA-cross-toolchain target)))
          (else '())))))
