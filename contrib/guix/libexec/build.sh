...
# Setup the directory where our aRIA Core build for HOST will occur
...
make -C depends --jobs="$MAX_JOBS" HOST="$HOST" \
                                   ...
                                   qt_config_opts_i686_linux='-platform linux-g++ -xplatform aRIA-linux-g++'
...
(
    ...
    # Setup the directory where our aRIA Core build for HOST will be
    # installed. This directory will also later serve as the input for our
    # binary tarballs.
    INSTALLPATH="${PWD}/installed/${DISTNAME}"
    mkdir -p "${INSTALLPATH}"
    # Install built aRIA Core to $INSTALLPATH
