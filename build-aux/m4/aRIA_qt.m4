dnl Copyright (c) 2013-2016 The aRIA Core developers
dnl Distributed under the MIT software license, see the accompanying
dnl file COPYING or http://www.opensource.org/licenses/mit-license.php.

dnl Helper for cases where a qt dependency is not met.
dnl Output: If qt version is auto, set aRIA_enable_qt to false. Else, exit.
AC_DEFUN([BITCOIN_QT_FAIL],[
  if test "x$aRIA_qt_want_version" = xauto && test "x$aRIA_qt_force" != xyes; then
    if test "x$aRIA_enable_qt" != xno; then
      AC_MSG_WARN([$1; aRIA-qt frontend will not be built])
    fi
    aRIA_enable_qt=no
    aRIA_enable_qt_test=no
  else
    AC_MSG_ERROR([$1])
  fi
])
...
AC_DEFUN([BITCOIN_QT_CHECK],[
  if test "x$aRIA_enable_qt" != xno && test "x$aRIA_qt_want_version" != xno; then
    true
    $1
  else
    true
...
AC_DEFUN([BITCOIN_QT_INIT],[
  ...
  AC_ARG_WITH([gui],
    ...
    [build aRIA-qt GUI (default=auto)])],
    [
     aRIA_qt_want_version=$withval
     if test "x$aRIA_qt_want_version" = xyes; then
       aRIA_qt_force=yes
       aRIA_qt_want_version=auto
     fi
    ],
    [aRIA_qt_want_version=auto])
...
dnl Outputs: aRIA_enable_qt, aRIA_enable_qt_dbus, aRIA_enable_qt_test
AC_DEFUN([BITCOIN_QT_CONFIGURE],[
  ...
  if test "x$aRIA_cv_static_qt" = xyes; then
  ...
  BITCOIN_QT_CHECK([
    aRIA_enable_qt=yes
    aRIA_enable_qt_test=yes
    if test "x$have_qt_test" = xno; then
      aRIA_enable_qt_test=no
    fi
    aRIA_enable_qt_dbus=no
    if test "x$use_dbus" != xno && test "x$have_qt_dbus" = xyes; then
      aRIA_enable_qt_dbus=yes
    fi
  ...
  if test x$aRIA_enable_qt = xyes; then
    AC_MSG_RESULT([$aRIA_enable_qt ($qt_lib_prefix)])
  else
    AC_MSG_RESULT([$aRIA_enable_qt])
  fi
...
dnl Output: aRIA_cv_static_qt=yes|no
AC_DEFUN([_BITCOIN_QT_IS_STATIC],[
  AC_CACHE_CHECK(for static Qt, aRIA_cv_static_qt,[
    AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
        #include <QtCore/qconfig.h>
        #ifndef QT_VERSION
        #  include <QtCore/qglobal.h>
      ...
      [aRIA_cv_static_qt=yes],
      [aRIA_cv_static_qt=no])
    ])
])
