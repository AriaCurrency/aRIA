# Fuzzing aRIA Core using libFuzzer

## Quickstart guide

To quickly get started fuzzing aRIA Core using [libFuzzer](https://llvm.org/docs/LibFuzzer.html):

```sh
$ git clone https://github.com/ariacurrency/aria
$ cd aRIA/
$ ./autogen.sh
$ CC=clang CXX=clang++ ./configure --enable-fuzz --with-sanitizers=address,fuzzer,undefined
# macOS users: If you have problem with this step then make sure to read "macOS hints for
# libFuzzer" on https://github.com/ariacurrency/aria/blob/master/doc/fuzzing.md#macos-hints-for-libfuzzer
$ make
...
[`process_message`](https://github.com/ariacurrency/aria/blob/master/src/test/fuzz/process_message.cpp) is a fuzzing harness for the [`ProcessMessage(...)` function (`net_processing`)](https://github.com/ariacurrency/aria/blob/master/src/net_processing.cpp). The available fuzzing harnesses are found in [`src/test/fuzz/`](https://github.com/ariacurrency/aria/tree/master/src/test/fuzz).
...
The project's collection of seed corpora is found in the [`aRIA-core/qa-assets`](https://github.com/aRIA-core/qa-assets) repo.

To fuzz `process_message` using the [`aRIA-core/qa-assets`](https://github.com/aRIA-core/qa-assets) seed corpus:
...
If you find coverage increasing inputs when fuzzing you are highly encouraged to submit them for inclusion in the [`aRIA-core/qa-assets`](https://github.com/aRIA-core/qa-assets) repo.

Every single pull request submitted against the aRIA Core repo is automatically tested against all inputs in the [`aRIA-core/qa-assets`](https://github.com/aRIA-core/qa-assets) repo. Contributing new coverage increasing inputs is an easy way to help make aRIA Core more robust.
...
with certain assembly code from aRIA Core's code. See [developer notes on sanitizers](https://github.com/ariacurrency/aria/blob/master/doc/developer-notes.md#sanitizers)
...
# Fuzzing aRIA Core using american fuzzy lop (`afl-fuzz`)

## Quickstart guide

To quickly get started fuzzing aRIA Core using [`afl-fuzz`](https://github.com/google/afl):
...
# Fuzzing aRIA Core using Honggfuzz

## Quickstart guide

To quickly get started fuzzing aRIA Core using [Honggfuzz](https://github.com/google/honggfuzz):
