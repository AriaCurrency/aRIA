# PSBT Howto for aRIA Core

Since aRIA Core 0.17, an RPC interface exists for Partially Signed aRIA
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/aRIA/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for aRIA transactions that are not fully signed
...
#### Multisig with multiple aRIA Core instances

Alice, Bob, and Carol want to create a 2-of-3 multisig address. They're all using
aRIA Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `aRIA-cli` is used.
