aRIA Core version 0.11.0 is now available from:

  <https://aRIA.org/bin/aRIA-core-0.11.0/>
...
For example, add the following to `aRIA.conf`:
...
related to the blockchain in the aRIA system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows aRIA Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that aRIA Core will 
...
For an example script see [aRIA-submittx](https://github.com/laanwj/aRIA-submittx).
...
- #5334 `cf87536` libaRIAconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new aRIA-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only aRIAAmountField
- #5753 `0f44672` Add aRIA logo to about screen
...
- #5809 `46bfbe7` Add aRIA-cli man page
...
- #5849 `41259ca` contrib/init/aRIAd.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename aRIA-Qt.app to aRIA-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for aRIA-tx)
