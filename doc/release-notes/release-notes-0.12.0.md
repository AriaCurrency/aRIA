aRIA Core version 0.12.0 is now available from:

  <https://aRIA.org/bin/aRIA-core-0.12.0/>
...
installer (on Windows) or just copy over /Applications/aRIA-Qt (on Mac) or
aRIAd/aRIA-qt (on Linux).
...
backwards-compatible with pre-0.10 versions of aRIA Core or other software:
...
pre-0.12 versions of aRIA Core or other software.

If you want to downgrade after you have done a reindex with 0.12.0 or later,
you will need to reindex when you first start aRIA Core version 0.11 or
earlier.
...
ECDSA signatures inside aRIA transactions now use validation using
[libsecp256k1](https://github.com/aRIA-core/secp256k1) instead of OpenSSL.
...
(https://github.com/aRIA/bips/blob/master/bip-0130.mediawiki)
...
Previous versions of aRIA Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of aRIA Core was to raise the default minimum
relay fee.

aRIA Core 0.12 will have a strict maximum size on the mempool. The
...
aRIA Core 0.12 also introduces new default policy limits on the length and
...
aRIA Core 0.12 nodes. aRIA Core will only allow replacement of
transactions which have any of their inputs' `nSequence` number set to less
than `0xffffffff - 1`.  Moreover, a replacement transaction may only be
accepted when it pays sufficient fee, as described in [BIP 125]
(https://github.com/aRIA/bips/blob/master/bip-0125.mediawiki).
...
Note that the wallet in aRIA Core 0.12 does not yet have support for
...
This allows running aRIAd without having to do any manual configuration.
...
aRIA Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. aRIA Core will relay transactions with
insufficient fees depending on the setting of `-limitfreerelay=<r>` (default:
`r=15` kB per minute) and `-blockprioritysize=<s>`.

In aRIA Core 0.12, when mempool limit has been reached a higher minimum
...
entirely in aRIA Core 0.13, and it is at this time undecided whether the
...
aRIA Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
aRIA Core automatically creates a hidden service to listen on, without
manual configuration. aRIA Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if aRIA Core is listening, and
...
aRIAd can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals aRIA
Core to use floating fees. By default, aRIA Core will use floating
fees.
...
At all times, aRIA Core will cap fees at `-maxtxfee=<x>` (default:
0.10) BTC.
Furthermore, aRIA Core will never create transactions paying less than
...
`aRIA.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
notes of v0.11.0](https://github.com/ariacurrency/aria/blob/v0.11.0/doc/release-notes.md#block-file-pruning).

`NODE_BLOOM` service bit
------------------------

Support for the `NODE_BLOOM` service bit, as described in [BIP
111](https://github.com/aRIA/bips/blob/master/bip-0111.mediawiki), has been
...
* OP_NOP2 has been renamed to OP_CHECKLOCKTIMEVERIFY by [BIP 65](https://github.com/aRIA/bips/blob/master/bip-0065.mediawiki)
...
caching. A sample config for apache2 could look like:
    ...
    <Location /aRIArpc>
        ...
        # optional bypass aRIAd rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< aRIArpc:<password>
    </Location>
...
Restarting aRIAd will no longer clear out the list of banned peers; instead
...
- #6214 `87406aa` [OSX] revert renaming of aRIA-Qt.app and use CFBundleDisplayName (partial revert of #6116) (Jonas Schnelli)
- #6218 `9d67b10` build/gitian misc updates (Cory Fields)
- #6269 `d4565b6` gitian: Use the new aRIA-detached-sigs git repo for OSX signatures (Cory Fields)
...
- #6600 `ef4945f` Include aRIA-tx binary on Debian/Ubuntu (Zak Wilcox)
...
- #6390 `6a73d66` tests: Fix aRIA-tx signing test case (Wladimir J. van der Laan)
...
- #6730 `cb4d6d0` build: Remove dependency of aRIA-cli on secp256k1 (Wladimir J. van der Laan)
...
- #6387 `11576a5` [aRIA-cli] improve error output (Jonas Schnelli)
- #6401 `6db53b4` Add BITCOIND_SIGTERM_TIMEOUT to OpenRC init scripts (Florian Schmaus)
- #6430 `b01981e` doc: add documentation for shared library libaRIAconsensus (Braydon Fuller)
