...
aRIA Core version 0.20.0 is now available from:

  <https://aRIAcore.org/bin/aRIA-core-0.20.0/>
...
#### Removal of BIP61 reject network messages from aRIA Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since aRIA Core version 0.18.0.
...
- Testing or debugging of implementations of the aRIA P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of aRIA Core. aRIA Core logs debug messages
...
- OpenSSL is no longer used by aRIA Core.  (#17265)

- BIP70 support has been fully removed from aRIA Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/ariacurrency/aria/issues/16599),
  [PR #16702](https://github.com/ariacurrency/aria/pull/16702), and the
  `aRIAd help` for more information.  This option is experimental and
...
- All custom settings configured when aRIA Core starts are now
...
- The "Start aRIA Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://aRIAcore.org/en/releases/0.18.0/#wallet-gui)
  in the release notes of version 0.18 has been fixed.  The issue
  affected anyone who simultaneously used multiple aRIA Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed aRIA Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/aRIA-core/HWI) for
  signing.  Future versions of aRIA Core should support a GUI option
...
- aRIA Core's automatically-generated source code documentation is
  now available at https://doxygen.aRIAcore.org.  (#17596)
...
- The `aRIA-cli` utility used with the `-getinfo` parameter now
...
- The `aRIA-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17068 Always generate `aRIAstrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_aRIA-qt` in msvc build (sipsorcery)
...
- #18397 Fix libevent linking for `bench_aRIA` binary (hebasto)
...
- #18702 Fix ASLR for aRIA-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate aRIA schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest aRIA.conf (sdaftuar)
...
- #18022 Fix appveyor `test_aRIA` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (aRIA-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone aRIA-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about aRIA Core files (hebasto)
...
- #17119 Fix broken aRIA-cli examples (andrewtoth)
...
- #17503 Remove bitness from aRIA-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.aRIAcore.org (laanwj)
...
- #17648 Rename wallet-tool references to aRIA-wallet (hel-o)
...
- #16115 On aRIAd startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from aRIA-cli -getinfo (malevolent)
