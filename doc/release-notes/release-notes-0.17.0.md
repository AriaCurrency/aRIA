aRIA Core version 0.17.0 is now available from:

  <https://aRIAcore.org/bin/aRIA-core-0.17.0/>
...
aRIA Core is extensively tested on multiple operating systems using
...
- `-includeconf=<file>` can be used to include additional configuration files.
  Only works inside the `aRIA.conf` file, not inside included files or from
  ...
  as aRIA.conf will still include `relative.conf`.
...
Previously, wallets could only be loaded or created at startup, by specifying `-wallet` parameters on the command line or in the aRIA.conf file. It is now possible to load, create and unload wallets dynamically at runtime:
...
be used in V0.17 by starting aRIAd with the '-deprecatedrpc=accounts'
...
BIP 174 Partially Signed aRIA Transactions support
-----------------------------------------------------

[BIP 174 PSBT](https://github.com/aRIA/bips/blob/master/bip-0174.mediawiki) is an interchange format for aRIA transactions that are not fully signed
yet, together with relevant metadata to help entities work towards signing it.
It is intended to simplify workflows where multiple parties need to cooperate to
produce a transaction. Examples include hardware wallets, multisig setups, and
[CoinJoin](https://aRIAtalk.org/?topic=279249) transactions.

### Overall workflow

Overall, the construction of a fully signed aRIA transaction goes through the
following steps:
...
- An **Extractor** produces a valid aRIA transaction (in network format)
...
- The new RPC `getzmqnotifications` returns information about active ZMQ
  notifications.
- When aRIA is not started with any `-wallet=<path>` options, the name of
  the default wallet returned by `getwalletinfo` and `listwallets` RPCs is
  now the empty string `""` instead of `"wallet.dat"`. If aRIA is started
...
- `signrawtransaction` is deprecated and will be fully removed in v0.18. To use
  `signrawtransaction` in v0.17, restart aRIAd with
...
- When running aRIAd with `-debug` but without `-daemon`, logging to stdout
...
meaning the `-txindex` flag can be toggled without a full reindex. If aRIAd
...
- #11962 `0a01843` add seed.aRIA.sprovoost.nl to DNS seeds (Sjors)
...
- #12723 `310dc61` Qt5: Warning users about invalid-BIP21 URI aRIA:// (krab)
...
- #13528 `b877c39` Move aRIAGUI initializers to class, fix initializer order warning (laanwj)
...
- #13486 `66e1a08` Move rpc/util.cpp from libaRIA-util to libaRIA-server (ken2812221)
...
- #13604 `75bea05` Add depends 32-bit arm support for aRIA-qt (TheCharlatan)
...
- #13696 `2ab7208` Add aarch64 qt depends support for cross compiling aRIA-qt (TheCharlatan)
...
- #11771 `2dbc4a4` Change invalidtxrequest to use aRIATestFramework (jnewbery)
...
- #12443 `07090c5` Move common args to aRIA.conf (MarcoFalke)
...
- #11772 `0630974` Change invalidblockrequest to use aRIATestFramework (jnewbery)
...
- #11773 `f0f9732` Change `feature_block.py` to use aRIATestFramework (jnewbery)
- #12839 `40f4baf` Remove travis checkout depth (laanwj)
- #11817 `2a09a78` Change `feature_csv_activation.py` to use aRIATestFramework (jnewbery)
...
- #12856 `27278df` Add Metaclass for aRIATestFramework (WillAyd)
...
- #12882 `d96bdd7` Make `test_aRIA` pass under ThreadSanitzer (clang). Fix lock-order-inversion (potential deadlock) (practicalswift)
...
- #13687 `9d26b69` travis: Check that ~/.aRIA is never created (MarcoFalke)
...
- #13811 `660abc1` travis: Run `bench_aRIA` once (MarcoFalke)
...
- #12102 `7fb8fb4` Apply hardening measures in aRIAd systemd service file (Flowdalic)
- #12652 `55f490a` aRIA-cli: Provide a better error message when aRIAd is not running (practicalswift)
...
- #12769 `35eb9d6` Add systemd service to aRIAd in debian package (ghost)
...
- #13374 `56f6936` utils and libraries: checking for aRIA address in translations (kaplanmaxe)
...
- #13603 `453ae5e` aRIA-tx: Stricter check for valid integers (domob1812)
...
- #13725 `07ce278` Fix aRIA-cli --version (Empact)
...
- #12877 `2b54155` Use aRIAd in Tor documentation (knoxcard)
...
- #13346 `60f0358` update aRIA-dot-org links in release-process.md (fanquake)
...
- #13742 `d2186b3` Adjust aRIAcore.org links (MarcoFalke)
...
- #13905 `73a09b4` fixed aRIA-cli -help output for help2man (hebasto)
