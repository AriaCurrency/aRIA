aRIA Core version 0.13.1 is now available from:

  <https://aRIA.org/bin/aRIA-core-0.13.1/>
...
updates will be released anymore. Without security updates, using a aRIA
wallet on a XP machine is irresponsible at least.

In addition to that, with 0.12.x there have been varied reports of aRIA Core
randomly crashing on Windows XP. It is [not clear](https://github.com/ariacurrency/aria/issues/7681#issuecomment-217439891)
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming aRIA wallet
...
- **Weighting data based on how it affects node performance:** Some parts of
  each aRIA block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of aRIAs they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of aRIAs being spent is exactly
...
- **Increased security for multisig:** aRIA addresses (both P2PKH addresses
  ...
  believed to be provided by aRIA's choice of parameters for its Elliptic
  Curve Digital Security Algorithm [ECDSA].)

- **More efficient almost-full-node security** Satoshi Nakamoto's original
  aRIA paper describes a method for allowing newly-started full nodes to
  skip downloading and validating some data from historic blocks that are
  protected by large amounts of proof of work.  Unfortunately, Nakamoto's
  method can't guarantee that a newly-started node using this method will
  produce an accurate copy of aRIA's current ledger (called the UTXO set),
  ...
  note that aRIA Core does not provide an option to use this capability as
  of this 0.13.1 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  aRIA users to individually opt-in to almost any change in the aRIA
  Script language when those users receive new transactions.  Features
  currently being researched by aRIA Core contributors that may use this
  capability include support for Schnorr signatures, which can improve the
  privacy and efficiency of multisig transactions (or transactions with
  multiple inputs), and Merklized Abstract Syntax Trees (MAST), which can
  improve the privacy and efficiency of scripts with two or more conditions.
  Other aRIA community members are studying several other improvements
...
[Segwit FAQ]: https://aRIAcore.org/en/2016/01/26/segwit-benefits/
[segwit wallet developers guide]: https://aRIAcore.org/en/segwit_wallet_dev/
[BIP141]: https://github.com/aRIA/bips/blob/master/bip-0141.mediawiki
[BIP143]: https://github.com/aRIA/bips/blob/master/bip-0143.mediawiki
[BIP144]: https://github.com/aRIA/bips/blob/master/bip-0144.mediawiki
[BIP145]: https://github.com/aRIA/bips/blob/master/bip-0145.mediawiki
[versionbits FAQ]: https://aRIAcore.org/en/2016/06/08/version-bits-miners-faq/
...
Since aRIA Core 0.10.0, nodes have defaulted to only relaying and
...
[BIP147]: https://github.com/aRIA/bips/blob/master/bip-0147.mediawiki
...
- `aRIA-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `aRIA-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
...
- #8848 `7a34a46` Add NULLDUMMY verify flag in aRIAconsensus.h (jl2012)
...
- #8940 `5b4192b` Add x9 service bit support to dnsseed.bluematt.me, seed.aRIAstats.com (TheBlueMatt, cdecker)
- #8944 `685e4c7` Remove bogus assert on number of oubound connections. (TheBlueMatt)
- #8949 `0dbc48a` Be more agressive in getting connections to peers with relevant services (gmaxwell)

### Build system
- #8293 `fa5b249` Allow building libaRIAconsensus without any univalue (luke-jr)
- #8492 `8b0bdd3` Allow building bench_aRIA by itself (luke-jr)
...
- #8829 `2a8bca4` Add aRIA-tx JSON tests (jnewbery)
- #8834 `1dd1783` blockstore: Switch to dumb dbm (MarcoFalke)
- #8835 `d87227d` nulldummy.py: Don't run unused code (MarcoFalke)
- #8836 `eb18cc1` aRIA-util-test.py should fail if the output file is empty (jnewbery)
...
- #8860 `0bee740` util: Move wait_aRIAds() into stop_nodes() (MarcoFalke)
...
- #8683 `8a7d7ff` Fix incorrect file name aRIA.qrc  (aRIAsSG)
...
- #8817 `bcf3806` update aRIA-tx to output witness data (jnewbery)
