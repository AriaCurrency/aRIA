aRIA Core version 0.19.0.1 is now available from:

  <https://aRIAcore.org/bin/aRIA-core-0.19.0.1/>
...
installer (on Windows) or just copy over `/Applications/aRIA-Qt` (on Mac)
or `aRIAd`/`aRIA-qt` (on Linux).

Upgrading directly from a version of aRIA Core that has reached its EOL is
possible, but might take some time if the datadir needs to be migrated.  Old
wallet versions of aRIA Core are generally supported.

Compatibility
==============

aRIA Core is supported and extensively tested on operating systems using
the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not recommended
to use aRIA Core on unsupported systems.
...
to open the aRIA Core .dmg. This is due to new signing requirements
imposed by Apple, which the aRIA Core project does not yet adhere too.
...
- [Reduce memory](https://github.com/ariacurrency/aria/blob/master/doc/reduce-memory.md)
  suggests configuration tweaks for running aRIA Core on systems with
...
- `-blockfilterindex` enables the creation of BIP158 block filters for
  ...
  currently use about 4 GiB of space.  Note: this version of aRIA
...
- `whitebind` and `whitelist` now accept a list of permissions to
  ...
  releases.  See the `aRIAd -help` output for these two options for
...
- In 0.18.0, a `./configure` flag was introduced to allow disabling BIP70 support in the GUI (support was enabled by default). In 0.19.0, this flag is now __disabled__ by default. If you want to compile aRIA Core with BIP70 support in the GUI, you can pass `--enable-bip70` to `./configure`. (#15584)
...
- `generate` is now removed after being deprecated in aRIA Core 0.18.
...
- BIP 61 reject messages were deprecated in v0.18. They are now disabled
  ...
  future version of aRIA Core. (#14054)

- To eliminate well-known denial-of-service vectors in aRIA Core,
  ...
  This prevents aRIA Core from sending the BIP111 NODE_BLOOM service
  ...
  of aRIA Core and nodes that have manually enabled BIP37 support,
...
- By default, aRIA Core will now make two additional outbound connections that are exclusively used for block-relay.  No transactions or addr messages will be processed on these connections. These connections are designed to add little additional memory or bandwidth resource requirements but should make some partitioning attacks more difficult to carry out. (#15759)

Miscellaneous CLI Changes
-------------------------

- The `testnet` field in `aRIA-cli -getinfo` has been renamed to
...
- `getblockchaininfo` no longer returns a `bip9_softforks` object.
  ...
  an additional `type` field describes how aRIA Core determines
...
- When fetching a transaction announced by multiple peers, previous versions of
  aRIA Core would sequentially attempt to download the transaction from each
...
- If a Tor hidden service is being used, aRIA Core will be bound to
...
- Transactions with outputs paying v1 to v16 witness versions (future
  ...
  and services can accept any valid bech32 aRIA address without
...
- A new wallet flag `avoid_reuse` has been added (default off). When
  ...
  partial spends" (added in aRIA Core v0.17.0), this can eliminate a
...
- #15649 Add ChaCha20Poly1305@aRIA AEAD (jonasschnelli)
...
- #15639 aRIA-wallet tool: Drop libaRIA_server.a dependency (ryanofsky)
...
- #16366 init: Use InitError for all errors in aRIAd/qt (MarcoFalke)
...
- #15239 scripts and tools: Move non-linux build source tarballs to "aRIA-binaries/version" directory (hebasto)
...
- #16258 [MSVC]: Create the config.ini as part of aRIAd build (NicolasDorier)
...
- #16441 remove qt libjpeg check from aRIA_qt.m4 (fanquake)
...
- #16750 msbuild: adds bench_aRIA to auto generated project files (sipsorcery)
...
- #15773 Add aRIATestFramework::sync_* methods (MarcoFalke)
...
- #15352 Reduce noise level in test_aRIA output (practicalswift)
...
- #15896 feature_filelock, interface_aRIA_cli: Use PACKAGE_NAME in messages rather than hardcoding aRIA Core (luke-jr)
...
- #16277 Suppress output in test_aRIA for expected errors (gertjaap)
...
- #16489 log: harmonize aRIAd logging (jonatack)
...
- #17449 fix uninitialized variable nMinerConfirmationWindow (aRIAVBR)

### Documentation
- #15514 Update Transifex links (fanquake)
- #15513 add "sections" info to example aRIA.conf (fanquake)
...
- #16080 build/doc: update aRIA_config.h packages, release process (jonatack)
...
- #16448 add note on precedence of options in aRIA.conf (fanquake)
...
- #16621 add default aRIA.conf locations (GChuf)
...
- #16863 Add a missing closing parenthesis in the aRIA-wallet's help (darosior)
- #16757 CChainState return values (MarcoFalke)
- #16847 add comments clarifying how local services are advertised (jamesob)
- #16812 Fix whitespace errs in .md files, aRIA.conf, and Info.plist.in (ch4ot1c)
...
- #16912 Remove Doxygen intro from src/aRIAd.cpp (ch4ot1c)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/aRIA/aRIA/).
