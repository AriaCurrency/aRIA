aRIA Core version 0.14.0 is now available from:

  <https://aRIA.org/bin/aRIA-core-0.14.0/>
...
aRIA Core has supported automatically pruning the blockchain since 0.11. Pruning
...
now ZMQ can be used on Windows. Please see [this document](https://github.com/ariacurrency/aria/blob/master/doc/zmq.md) for
...
When aRIA Core is out-of-sync on startup, a semi-transparent information
...
Commands sent over the JSON-RPC interface and through the `aRIA-cli` binary
can now use named arguments. This follows the [JSON-RPC specification](http://www.jsonrpc.org/specification)
for passing parameters by-name with an object.

`aRIA-cli` has been updated to support this by parsing `name=value` arguments
when the `-named` option is given.

Some examples:

    src/aRIA-cli -named help command="help"
    src/aRIA-cli -named getblockhash height=0
    src/aRIA-cli -named getblock blockhash=000000000019d6689c085ae165831e934ff763ae46a2a6c172b3f1b60a8ce26f
    src/aRIA-cli -named sendtoaddress address="(snip)" amount="1.0" subtractfeefromamount=true
...
The Alert System was [disabled and deprecated](https://aRIA.org/en/alert/2016-11-01-alert-retirement) in aRIA Core 0.12.1 and removed in 0.13.0. 
...
----------------------
 ...
 - A new RPC command `getmemoryinfo` has been added which will return information
   about the memory usage of aRIA Core. This was added in conjunction with
   optimizations to memory management. See [Pull #8753](https://github.com/ariacurrency/aria/pull/8753)
...
- #8722 `bb843ad` aRIA-cli: More detailed error reporting (laanwj)
...
- #8830 `ef0801b` test: Add option to run aRIA-util-test.py manually (jnewbery)
- #8881 `e66cc1d` Add some verbose logging to aRIA-util-test.py (jnewbery)
...
- #9023 `774db92` Add logging to aRIA-util-test.py (jnewbery)
- #9065 `c9bdf9a` Merge `doc/unit-tests.md` into `src/test/README.md` (laanwj)
- #9069 `ed64bce` Clean up bctest.py and aRIA-util-test.py (jnewbery)
...
- #9691 `fc67cd2` Init ECC context for `test_aRIA_fuzzy` (gmaxwell)
...
- #9865 `289204f` Change aRIA address in RPC help message (marijnfs)
...
- #8813 `fb24d7e` aRIAd: Daemonize using daemon(3) (laanwj)
...
- #8837 `2108911` Allow aRIA-tx to parse partial transactions (jnewbery)
- #9204 `74ced54` Clarify CreateTransaction error messages (instagibbs)
- #9265 `31bcc66` aRIA-cli: Make error message less confusing (laanwj)
...
- #8883 `d5d4ad8` Add all standard TXO types to aRIA-tx (jnewbery)
