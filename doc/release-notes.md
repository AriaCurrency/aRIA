...
aRIA Core version 0.19.0 is now available from:

  <https://aRIAcore.org/bin/aRIA-core-0.19.0/>
=======
0.21.0 Release Notes
====================

aRIA Core version 0.21.0 is now available from:

  <https://aRIAcore.org/bin/aRIA-core-0.21.0/>
...
using the Linux kernel, macOS 10.12+, and Windows 7 and newer.  aRIA
Core should also work on most other Unix-like systems but is not as
frequently tested on them.  It is not recommended to use aRIA Core on
unsupported systems.

From aRIA Core 0.20.0 onwards, macOS versions earlier than 10.12 are no
longer supported. Additionally, aRIA Core does not yet change appearance
...
- This release adds support for Tor version 3 hidden services, and rumoring them
  ...
  [BIP155](https://github.com/aRIA/bips/blob/master/bip-0155.mediawiki).
  Version 2 hidden services are still fully supported by aRIA Core, but the
...
- The Tor onion service that is automatically created by setting the
  ...
  `-datadir`) and can be removed if not needed. aRIA Core will no longer
...
- This release adds support for serving
  [BIP157](https://github.com/aRIA/bips/blob/master/bip-0157.mediawiki) compact
...
- This release adds support for signets
  ([BIP325](https://github.com/aRIA/bips/blob/master/bip-0325.mediawiki)) in
...
- This release implements
  [BIP339](https://github.com/aRIA/bips/blob/master/bip-0339.mediawiki)
...
- This release implements the proposed Taproot consensus rules
  ([BIP341](https://github.com/aRIA/bips/blob/master/bip-0341.mediawiki) and
  [BIP342](https://github.com/aRIA/bips/blob/master/bip-0342.mediawiki)),
...
- A new `aRIA-cli -netinfo` command provides a network peer connections
...
- A new `aRIA-cli -generate` command, equivalent to RPC `generatenewaddress`
  followed by `generatetoaddress`, can generate blocks for command line testing
  purposes. This is a client-side version of the former `generate` RPC. See the
  help for details. (#19133)

- The `aRIA-cli -getinfo` command now displays the wallet name and balance for
  each of the loaded wallets when more than one is loaded (e.g. in multiwallet
  mode) and a wallet is not specified with `-rpcwallet`. (#18594)

- The `connections` field of `aRIA-cli -getinfo` is now expanded to return a JSON
...
- The `startupnotify` option is used to specify a command to
  execute when aRIA Core has finished with its startup
  sequence. (#15367)
...
- The `-salvagewallet` startup option has been removed. A new `salvage` command
  has been added to the `aRIA-wallet` tool which performs the salvage
...
aRIA Core will no longer automatically create new wallets on startup. It will
load existing wallets specified by `-wallet` options on the command line or in
`aRIA.conf` or `settings.json` files. And by default it will also load a
top-level unnamed ("") wallet. However, if specified wallets don't exist,
aRIA Core will now just log warnings instead of creating new wallets with
new keys and addresses like previous releases did.

New wallets can be created through the GUI (which has a more prominent create
wallet option), through the `aRIA-cli createwallet` or `aRIA-wallet
...
Bugs and missing functionality can be reported to the [issue tracker](https://github.com/ariacurrency/aria/issues).
...
Users who already have a aRIA Core wallet can continue to use it as they did before without
any change in behavior. Newly created Legacy Wallets (which remains the default type of wallet) will
behave as they did in previous versions of aRIA Core.
...
The change to using descriptors changes the default derivation paths used by aRIA Core
...
- Wallets created or loaded in the GUI will now be automatically loaded on
  startup, so they don't need to be manually reloaded next time aRIA Core is
  started. The list of wallets to load on startup is stored in
  `\<datadir\>/settings.json` and augments any command line or `aRIA.conf`
...
- #19284 Add seed.aRIA.wiz.biz to DNS seeds (wiz)
...
- #20198 Show name, format and if uses descriptors in aRIA-wallet tool (jonasschnelli)
...
- #19133 rpc, cli, test: add aRIA-cli -generate command (jonatack)
...
- #20055 Set HTTP Content-Type in aRIA-cli (laanwj)
...
### Build system
- #18504 Drop aRIA-tx and aRIA-wallet dependencies on libevent (ryanofsky)
...
- #18653 add coverage for aRIA-cli -rpcwait (jonatack)
...
- #18739 Document how to fuzz aRIA Core using Honggfuzz (practicalswift)
...
- #19061 Add link to Visual Studio build readme (maitreaRIA)
...
- #18817 Document differences in aRIAd and aRIA-qt locale handling (practicalswift)
...
[Transifex](https://www.transifex.com/aRIA/aRIA/).
