...
The aRIA Core daemon can be configured to act as a trusted "border
router", implementing the aRIA wire protocol and relay, making
...
The ZeroMQ feature in aRIA Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building aRIAd:
...
For instance:
    ...
    $ aRIAd -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/aRIAd.tx.raw \
...
These options can also be provided in aRIA.conf.
...
From the perspective of aRIAd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into aRIAd directly. Furthermore, no information is
...
using. aRIAd appends an up-counting sequence number to each
