...
void ReceiveCoinsDialog::setModel(WalletModel *_model)
{
    ...
    if(_model && _model->getOptionsModel())
    {
        ...
        ui->addressTypeSelector->setModel(new aRIAAddressTypes(this));
        
        // user explicitly set the type, use it
        if (model->wallet().getDefaultAddressType() == OutputType::BECH32) {
            ui->addressTypeSelector->setCurrentIndex(aRIAAddressType::NATIVE_SEGWIT);
        } else if (model->wallet().getDefaultAddressType() == OutputType::P2SH_SEGWIT){
            ui->addressTypeSelector->setCurrentIndex(aRIAAddressType::NESTED_SEGWIT);
        } else {
            ui->addressTypeSelector->setCurrentIndex(aRIAAddressType::LEGACY);
        }
...
void ReceiveCoinsDialog::on_receiveButton_clicked()
{
    ...
    aRIAAddressType address_type_selected = (aRIAAddressType)ui->addressTypeSelector->currentIndex();
    if (aRIAAddressType::NATIVE_SEGWIT == address_type_selected) {
        address_type = OutputType::BECH32;
    } else if (aRIAAddressType::NESTED_SEGWIT == address_type_selected) {
...
void ReceiveCoinsDialog::copyURI()
{
    ...
    const QString uri = GUIUtil::formataRIAURI(submodel->entry(sel.row()).recipient);
