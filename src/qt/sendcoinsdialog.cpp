...
void SendCoinsDialog::setModel(WalletModel *_model)
{
    ...
    if(_model && _model->getOptionsModel())
    {
        ...
        connect(ui->customFee, &aRIAAmountField::valueChanged, this, &SendCoinsDialog::coinControlUpdateLabels);
        ...
        if (model->wallet().privateKeysDisabled()) {
            ...
            ui->sendButton->setToolTip(tr("Creates a Partially Signed aRIA Transaction (PSBT) for use with e.g. an offline %1 wallet, or a PSBT-compatible hardware wallet.").arg(PACKAGE_NAME));
        }
...
bool SendCoinsDialog::PrepareSendText(QString& question_string, QString& informative_text, QString& detailed_text)
{
    ...
    processSendCoinsReturn(prepareStatus,
        aRIAUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), m_current_transaction->getTransactionFee()));
    ...
    for (const SendCoinsRecipient &rcp : m_current_transaction->getRecipients())
    {
        ...
        QString amount = aRIAUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), rcp.amount);
    ...
    if (model->wallet().privateKeysDisabled()) {
        question_string.append(tr("Please, review your transaction proposal. This will produce a Partially Signed aRIA Transaction (PSBT) which you can save or copy and then sign with e.g. an offline %1 wallet, or a PSBT-compatible hardware wallet.").arg(PACKAGE_NAME));
    } else {
    ...
    if(txFee > 0)
    {
        ...
        question_string.append(aRIAUnits::formatHtmlWithUnit(model->getOptionsModel()->getDisplayUnit(), txFee));
...
void SendCoinsDialog::on_sendButton_clicked()
{
    ...
    if (forDelegation && model->wallet().wallet()->GetLastBlockHeight() < Params().GetConsensus().RIAColdStakeEnableHeight)
    {
        Q_EMIT message(tr("Cold Staking not enabled"), tr("Cold staking is enabled at block %1").arg(Params().GetConsensus().RIAColdStakeEnableHeight), CClientUIInterface::MSG_ERROR);
        return;
    }
...
void SendCoinsDialog::setBalance(const interfaces::WalletBalances& balances)
{
    if(model && model->getOptionsModel())
    {
        ...
        ui->labelBalance->setText(aRIAUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), balance));
    }
}
...
void SendCoinsDialog::processSendCoinsReturn(const WalletModel::SendCoinsReturn &sendCoinsReturn, const QString &msgArg)
{
    ...
    case WalletModel::AbsurdFee:
        msgParams.first = tr("A fee higher than %1 is considered an absurdly high fee.").arg(aRIAUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), model->wallet().getDefaultMaxTxFee()));
        break;
...
void SendCoinsDialog::updateFeeMinimizedLabel()
{
    ...
    if (ui->radioSmartFee->isChecked())
        ...
        ui->labelFeeMinimized->setText(aRIAUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), ui->customFee->value()) + "/kB");
    }
}
