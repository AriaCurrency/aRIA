This directory contains the aRIAQT graphical user interface (GUI). It uses the cross-platform framework [Qt](https://www1.qt.io/developers/).
...
Represents the main window of the aRIA UI.
...
ClientModel is used by the main application `aRIAgui` and several models like `peertablemodel`.
...
Used to process BIP21 payment URI requests. Also handles URI based application switching (e.g. when following a aRIA:... link from a browser).
...
* UI elements like aRIAAmountField, which inherit from QWidget.
* `aRIAstrings.cpp`: automatically generated
* `aRIAunits.(h/cpp)`: RIA / mRIA / etc handling
* `callback.h`
...
* don't change `local/aRIA_en.ts`; this happens [automatically](/doc/translation_process.md#writing-code-with-translations)
...
10. Start debugging with Qt Creator (you might need to the executable to "aRIA-qt" under "Run", which is where you can also add command line arguments)
