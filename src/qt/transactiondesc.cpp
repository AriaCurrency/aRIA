...
QString TransactionDesc::toHTML(interfaces::Node& node, interfaces::Wallet& wallet, TransactionRecord *rec, int unit)
{
    ...
    if ((wtx.is_coinbase || wtx.is_coinstake) && nCredit == 0)
    {
        ...
        if (status.is_in_main_chain)
            strHTML += aRIAUnits::formatHtmlWithUnit(unit, nUnmatured)+ " (" + tr("matures in %n more block(s)", "", status.blocks_to_maturity) + ")";
        else
    ...
    else if (nNet > 0)
    {
        if (wtx.is_coinstake)
        {
            ...
            // Credit
            //
            strHTML += "<b>" + tr("Credit") + ":</b> " + aRIAUnits::formatHtmlWithUnit(unit, nCredit);
            ...
            // Debit
            //
            strHTML += "<b>" + tr("Debit") + ":</b> " + aRIAUnits::formatHtmlWithUnit(unit, -nDebit) + "<br>";
        }
        else
        {
            //
    ...
    // Message from normal aRIA:URI (aRIA:123...?message=example)
